...
struct Person {
  ...
  unsigned int nPool;
     bool sampled;//whether this person is sampled
    bool knownPositive;//wheter this sampled person is test positive
    //unsigned int testpoolId;//pool id
  ...
  friend inline void clearNeedsBoost(Person &p) {p.status|=NEEDSBOOST;}
    ...
    friend inline bool isSampled(const Person &p){return p.sampled&SAMPLED; }
    friend inline void cleanSampled(Person &p){p.sampled&=0; }
    friend inline void setIsSampled(Person &p){p.sampled|=SAMPLED;}
    friend inline bool isKnownPositive(const Person &p){return p.knownPositive;}
    friend inline void setKnownPositive(Person &p){p.knownPositive=1;  }
...
struct Community {
  ...
  int ninfS[TAG]; // number of residents currently infected and sampled; 
...
struct Tract {
  ...
  friend istream& operator>>(istream& is, Tract& t) {
    ...
    friend inline enum antiviralPolicy getAVPolicySampled(const Tract &t) { return (enum antiviralPolicy)(t.status&0x03); }
     friend inline void setAVPolicySampled(Tract &t, enum antiviralPolicy p) { t.status|=p;}
...
class EpiModel {
 ...
 protected:
  ...
  vector < unsigned int > nNumInfectedSampledHistory;// history of number of infected and sampled (across all nodes)
  vector < unsigned int > nNumInfectedHistory;// history of number of infected (across all nodes)
  vector < unsigned int > nNumCumulativeInfectedHistory;//history of number of cumulative infected (across all nodes)   
   vector < unsigned int > nNumInfectedSampledDailyHistory;//history of number of infected and sampled daily
...
//sample parameters
  double  fKnownPositiveFraction;
  double fknownPositiveIsolation;
  int sampledSize;
     int sampledDay;
  int poolSize;
  int knownisolated;
  int nNumInfectedSampleddaily;
  ...
  antiviralPolicy eAVPolicySampled; //for B
 ...
 double fQuarantineComplianceSampled;     // probability of individual compliance (set to 0 for no quarantine)
