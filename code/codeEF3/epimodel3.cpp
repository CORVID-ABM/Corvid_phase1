   ...
   //sample the population; if this person is infected, then mark it as isSampled, then this infected one will go through isolation method
      ...
      if(sampledDay > 0? (nTimer >= nTriggerTime-3 && (nTimer-nTriggerTime+3)%(sampledDay*2)==0):(nTimer == nTriggerTime-3)){
          ...
          nNumInfectedSampleddaily = 0;
            cout<<nTimer<<endl;
/*
              unordered_map<string, vector<int> > groups_map;
              unordered_map<string, vector<int> >:: iterator groupit;
        for (vector< Community >::iterator it = commvec.begin();
               it != commvec.end();
               it++) {
              Community &comm = *it;
             //create a shuffled person id order and group people
              vector<int> shuffleArr;
              for(unsigned int a = comm.nFirstPerson; a < comm.nLastPerson; a++) shuffleArr.push_back(a);
              random_shuffle(shuffleArr.begin(),shuffleArr.end());
              unsigned tid = 0; 
             for(vector<int>::iterator i = shuffleArr.begin(); i!=shuffleArr.end();++i){
                int sid = *i;
                Person &p = pvec[sid];
                p.groupId = tid/2;//2 is the number of people in each group
                tid++; 
                string key; 
                stringstream out;
                out << comm.id;
                ...
                out << p.groupId;
                key = out.str();
                groupit = groups_map.find(key);
                if(groupit == groups_map.end()) {
                      vector<int> idvec;
                      idvec.push_back(p.id);
                      groups_map[key] = idvec;//add pid
                 }
                else groups_map[key].push_back(p.id);
              }
             }
              double pool = (double)sampledSize/groups_map.size();
              cout << pool << " pool size: " << groups_map.size()<<endl;
              groupit = groups_map.begin();
             //get one random pool
              int random_index = 0;
              for (int poolcount=0; poolcount < sampledSize; poolcount++){ 
                   random_index = rand() % groups_map.size();
                   groupit = groups_map.begin();//reset the point to first element
                   advance(groupit, random_index);//randomly pick a pool
                   //cout << random_index <<" " << groupit->first << " " <<endl;
                   vector<int> pids = groupit->second;
                   //go through the person id
                   bool isAllPool = false;
                   for(vector<int>::iterator pit=pids.begin();pit!=pids.end(); pit++){ 
                       Person &person = pvec[*pit];
                       if(isInfected(person)) isAllPool = true;
                       cout << person.id<<": "<<isInfected(person) <<endl;
                   }
                   cout<<isAllPool<<endl;
                   if(isAllPool) {
                      for(vector<int>::iterator pit=pids.begin();pit!=pids.end(); pit++){ 
                        Person &person = pvec[*pit];
                         //set all person in this pool as sampled to isolatation                    
                              setIsSampled(person);//which means sampled people is affected and will go to isolation method
                              nsampleandinfected++;
                              person.knownPositive = 1;
                              ++commvec[person.nHomeComm].ninfS[person.age];
                              setWillBeAscertained(person);  
                             cout<<"iso: "<<person.groupId<<" " <<person.id<<endl;
                        }
  ...
  for (vector< Community >::iterator it = commvec.begin();
              ...
              Community &comm = *it;
               ...
               int Z = sampledSize/nNumCommunities;
              //cout << comm.id <<endl;
              for (unsigned int pid=comm.nFirstPerson;pid<comm.nLastPerson;pid++) {
                  Person &p = pvec[pid];
                 // cout << p.id <<" "<< isInfected(p) << endl;
                  cleanSampled(p); 
                      if(get_rand_double<=(double)Z/(double)comm.nNumResidents){
                          nsampled++;
                          if(isInfected(p)) { cout<<p.id<<endl;
