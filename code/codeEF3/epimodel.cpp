             ...
             //create a shuffled person id order and group people
              ...
              random_shuffle(shuffleArr.begin(),shuffleArr.end());
               ...
               for(vector<int>::iterator i = shuffleArr.begin(); i!=shuffleArr.end();++i){
                int sid = *i;
                Person &p = pvec[sid];
                p.nPool = tid/poolSize;//2 is the number of people in each group
                tid++; 
                string key; 
                stringstream out;
                out << comm.id;
                ...
                if(groupit == groups_map.end()) {
                      ...
                      randomvec.push_back(key);//keep the key 
                 }
                else groups_map[key].push_back(p.id);
              }
             }
            int gsize = groups_map.size();
          for (int poolcount=0; poolcount < gsize; poolcount++){ 
              randomindex.push_back(poolcount);
           }
            random_shuffle(randomindex.begin(),randomindex.end()); 

              // double pool = (double)sampledSize/groups_map.size();
              //cout << nTimer<< " pool size: " << groups_map.size()<<endl;
              groupit = groups_map.begin();
             //get one random pool
              int random_index = 0;
              for (int poolcount=0; poolcount < sampledSize; poolcount++){ 
                   //random_index = rand() % groups_map.size();
                   random_index = randomindex[poolcount];
                    //cout<<random_index<<endl;
                    vector<int> pids  = groups_map[randomvec[random_index]];
                   //groupit = groups_map.begin();//reset the point to first elem$
                   ...
                   if(isAllPool) {
                      for(vector<int>::iterator pit=pids.begin();pit!=pids.end();pit++){
                        Person &person = pvec[*pit];
                         //set all person in this pool as sampled to isolate later
                              setIsSampled(person);//which means sampled people will be isolated later
                              nsampleandinfected++;

                              person.knownPositive = 1;
                              ++commvec[person.nHomeComm].ninfS[person.age];
                              setWillBeAscertained(person);  
                            // cout<<"iso: "<<person.nPool<<" " <<person.id<<endl;
                        }
