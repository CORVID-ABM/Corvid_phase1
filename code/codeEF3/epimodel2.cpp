   ...
   //sample the population; if this person is infected, then mark it as isSampled, then this infected one will go through isolation method
      ...
      if(sampledDay > 0? (nTimer >= nTriggerTime-3 && (nTimer-nTriggerTime+3)%(sampledDay*2)==0):(nTimer == nTriggerTime-3)){
          ...
          nNumInfectedSampleddaily = 0;
              ...
              unordered_map<string,string> groups_map;
              unordered_map<string,string>:: iterator groupit;
        for (vector< Community >::iterator it = commvec.begin();
               it != commvec.end();
               it++) {
              Community &comm = *it;
             ...
             for(vector<int>::iterator i = shuffleArr.begin(); i!=shuffleArr.end();++i){
                ...
                out<< p.id;
                id_value = out.str();
                if(groupit == groups_map.end()) groups_map[key] = id_value;//add pid
                else groups_map[key] = groupit->second +";"+id_value;
                //cout<<p.id<<" gid "<<p.groupId<<endl;
              }
             }
             ...
             //get one random pool
              ...
              for (int poolcount=0; poolcount < sampledSize; poolcount++){ 
                   ...
                   advance(groupit, random_index);
                   cout << random_index <<" " << groupit->first << " "<< groupit->second <<endl;
                   string pids = groupit->second;
                   int pn = pids.length()+1;
                   char *pidarray = new char[pn];
                    strcpy(pidarray, pids.c_str());
                   //parse the person id
                   char *token;
                   char *rest = pidarray;
                   bool isAllPool = false;
                   while((token=strtok_r(rest,";", &rest))) {
                         stringstream pidstring(token);
                         int pid = 0;
                         pidstring >> pid;
                         Person &p = pvec[pid];
                         cout << p.id << endl;
                         if(isInfected(p)) isAllPool = true;
                   }
                   if(isAllPool) {
                      cout << " " <<endl;
                   }
