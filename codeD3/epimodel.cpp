...
EpiModel::EpiModel(EpiModelParameters &params) {
  ...
  memcpy(nSchoolOpeningDays, params.getSchoolOpeningDays(), 56*sizeof(int));
    ...
    fKnownPositiveFraction=params.getKnownPositiveFraction();
    fknownPositiveIsolation=params.getKnownPositiveIsolation();
    sampledSize=params.getSampledSize();
    AVEiSampled=params.getAVEiSampled();
    sampledDay=params.getSampledDay();
  ...
  eAVPolicySampled=params.getAVPolicySampled(); 
 fAdultEssentialFraction = params.getAdultEssentialFraction();
  ...
  fQuarantineComplianceSampled=params.getQuarantineComplianceSampled();
...
void EpiModel::read_workflow(void)
{
  ...
  // Read workflow data
  unsigned int *flow = (unsigned int *)malloc(sizeof(unsigned int)*nNumTracts*nNumTractsTotal); // Workerflow matrix
  if (!flow) {
...
void EpiModel::create_families(Community& comm, int nTargetSize) {
 ...
 memset(comm.ninfS, 0, TAG*sizeof(int));
...
/*
 * TAPSampled, for sampled & infected, to get treatment;
 * will use antiviralpolicySampled as treatment
 * TAP person p (antivirals to person and family)
 */
void EpiModel::TAPSampled(Person& p) {
  Community c = commvec[p.nDayComm];
  if (isAVProphylaxis(p))
    clearAVProphylaxis(p); // this person was preparing to get prophylaxis, but now should get treatment
  enum antiviralPolicy policy = getAVPolicySampled(tractvec[c.nTractID-nFirstTract]);
  if (policy==HHTAP || policy==FULLTAP || (policy==HHTAP100&&nNumTAPDone<100)) {
    nNumTAPDone++;
    for (unsigned int i=c.nFirstPerson; i<c.nLastPerson; i++) {
      Person &target = pvec[i];
...
void EpiModel::day(void) {
  ...
  for (vector< Person >::iterator it = pvec.begin(); 
    ...
    if (isAntiviral(p)) {  // effects of the antiviral on transmission
      p.prs *= (1.0-AVEs);
     // if(isSampled(p)) p.pri *= (1.0-AVEiSampled);
      p.pri *= (1.0-AVEi);
    }
    if (isSymptomatic(p))
  ...
  for (vector< Community >::iterator it = commvec.begin(); 
	...
	if (needsBoost(p)) {
      ...
      if (isSymptomatic(p)) p.pri *= fRelativeSymptomaticInfectiousness;
      assert(p.pri<=1.0);
      assert(p.prs<=1.0);
    }
  }
...
//cout<<"update infection:"<<knownisolated<<endl;
  vector< Community >::iterator cend = commvec.end();
  for (vector< Community >::iterator it = commvec.begin(); 
       it != cend;
       it++) {
    ...
    for (unsigned int pid=comm.nFirstPerson;
	 ...
	 pid++) {
      ...
      if (isInfected(p)) {
 	//Scenario B isolate people if they infected
        if(isSampled(p)){ 
          //if (isAscertainedIsolation(tractvec[comm.nTractID-nFirstTract]) ){//has infected and is sampled will be isolated
                //&& get_rand_double<fAscertainedIsolationCompliance) { // ascertained isolation
            { setWithdrawDays(p,getIncubationDays(p)+nAscertainmentDelay);
		//cout << p.id << ":"<<commvec[p.nHomeComm].nEverInfected[p.age]<<":"<< commvec[p.nHomeComm].ninfS[p.age] <<endl;
               // if (p.iday==getIncubationDays(p)+nAscertainmentDelay && getWillBeAscertained(p)) 
               TAPSampled(p);//the infected sampled get treatment
              ...
              if (isQuarantine(tractvec[comm.nTractID-nFirstTract])) {
                    for (unsigned int pid2=comm.nFirstPerson;
                         ...
                         pid2++) {
                      Person &p2 = pvec[pid2];
                      if (p.family==p2.family && p.id!=p2.id && get_rand_double<fQuarantineComplianceSampled) { // quarantine family member
	...
	if (isInfectious(p)) {
	  ...
	  if (isSymptomatic(p)&&!isSampled(p)) {
	    ...
	    if (getAVPolicy(tractvec[comm.nTractID-nFirstTract])!=NOAV) TAP(p);//treatment for symptomatic person
	...
	if (p.iday>=VLOADNDAY) {
	  ...
	  comm.ninf[p.age]--;
          //if(isSampled(p)) comm.ninfS[p.age]--;
	  p.iday=0;
	}
      }
      ...
      if (--p.nTravelTimer<=0) {
	{
	  ...
	  if (isInfected(p)) { // got sick on vacation
	    if (!isInfected(original)) {
	      ...
	      if(isSampled(p)) {
		 ++commvec[original.nHomeComm].ninfS[original.age];	
	         p.knownPositive = 1;
		}
		cout<<"travel infection"<<endl;
...
void EpiModel::response(void) {
  ...
  if (bTrigger && nTimer==nTriggerTime) {
	...
	setAVPolicy(t, eAVPolicy);         // start using antivirals in tract
      if (eAVPolicySampled!=NOAV && getAVPolicySampled(t)!=eAVPolicySampled)
           setAVPolicySampled(t, eAVPolicySampled);         // start using antivirals in tract
      if ( fQuarantineComplianceSampled>0.0 &&!isQuarantine(t)) setQuarantine(t);
      if ( fQuarantineCompliance>0.0 &&!isQuarantine(t)) setQuarantine(t);  // activate household quarantine in tract
      if ( !isAscertainedIsolation(t))
...
//      int nsampled = 0, nsampleandinfected= 0;
//      vector< Person >::iterator pend=pvec.end();
//        for (vector< Person >::iterator it = pvec.begin();
//         it != pend;
//         it++) {
//            Person &p = *it;
//            if(get_rand_double<=(double)sampledSize/(double)nNumPerson){
//                setIsSampled(p);
//                nsampled++;
//	       if(isInfected(p)) {/
//		nsampleandinfected++;
//                  p.knownPositive = 1;
//                 ++commvec[p.nHomeComm].ninfS[p.age];		
//		}
//            }  
//        }

//	sampledSize = nsampled;
//	cout << "sampled pop: "<< sampledSize << " infected&isolated: "<< nsampleandinfected <<endl;
...
void EpiModel::summary(void) {
  ...
  outfile << "Sampled population: " << sampledSize << endl;
  outfile << "Sampled population every(day): " << sampledDay << endl;
 
  {
    ...
    outfile << "Know positive isolation: " << fknownPositiveIsolation << endl;
    
    outfile << "Quarantine: " << fQuarantineCompliance << endl;
       outfile << "Quarantine sampled and positive: " << fQuarantineComplianceSampled << endl;
    ...
    if (eAVPolicy==NOAV)
      ...
      outfile << "Antiviral policy for know positive: ";
         if (eAVPolicySampled==NOAV)
           outfile << "none" << endl;
         else if (eAVPolicySampled==HHTAP)
           outfile << "HHTAP" << endl;
         else if (eAVPolicySampled==HHTAP100)
           outfile << "HHTAP100" << endl;
         else if (eAVPolicySampled==FULLTAP)
           outfile << "FULLTAP" << endl;
         else if (eAVPolicySampled==TREATMENTONLY)
           outfile << "treatmentonly" << endl;
         else
           outfile << "invalid strategy" << endl;
    if (nNumTAPDone>0)
...

  {
    ...
    outfile << "AVEiSampledAndPositive: " << AVEiSampled << endl;
    ...
    for (vector< unsigned int >::iterator it = nNumCumulativeSymptomaticHistory.begin();
	 ...
	 it++)
      outfile << *it << ",";
    outfile << endl;
  outfile << "Number infected (daily): ";
    for (vector< unsigned int >::iterator it = nNumInfectedHistory.begin();
         it != nNumInfectedHistory.end();
         it++)
      outfile << *it << ",";
    outfile << endl;

outfile << "Cumulative infected (daily): ";
    for (vector< unsigned int >::iterator it = nNumCumulativeInfectedHistory.begin();
         it != nNumCumulativeInfectedHistory.end();
         it++)
      outfile << *it << ",";
    outfile << endl;

  outfile << "Number infected and sampled (daily): ";
    for (vector< unsigned int >::iterator it = nNumInfectedSampledDailyHistory.begin();
         it != nNumInfectedSampledDailyHistory.end();
         it++)
      outfile << *it << ",";
    outfile << endl;

  outfile << "Cumulative infected and sampled (daily): ";
    for (vector< unsigned int >::iterator it = nNumInfectedSampledHistory.begin();
         it != nNumInfectedSampledHistory.end();
         it++)
      outfile << *it << ",";
    outfile << endl;
  }
 ...
 unsigned int nTotalInfSample[TAG];    // number of people infected and sampled
  unsigned int nTotal[TAG];	// number of people
  memset(nTotalSym, 0, sizeof(unsigned int)*TAG);
  memset(nTotalInfSample, 0,sizeof(unsigned int)*TAG);
  ...
  for (vector< Tract >::iterator it = tractvec.begin();
    ...
    int ntot[TAG];	// number of people
    int ninfs[TAG];      // number of people infected and sampled
    memset(nsym, 0, sizeof(int)*TAG);
    memset(ninf, 0, sizeof(int)*TAG);
    memset(ntot, 0, sizeof(int)*TAG);
    memset(ninfs, 0, sizeof(int)*TAG);
  ...
  for (vector< Person >::iterator it = pvec.begin(); 
       ...
       it++) {
    Person &p = *it;
    if(isSampled(p)&&isKnownPositive(p)) knownisolated++;
    if (!isSusceptible(p) && getWillBeSymptomatic(p) && (!isInfected(p) ||    // was sick
                ...
                if (isVaccinated(p)) nVacSymptomatic[whichVaccine(p)*TAG+p.age]++;
                else nUnvacSymptomatic[p.age]++;
      if (isPregnant(p))
  ...
  }
  {
    ...
    outfile << endl;
    unsigned int ni=0, ns=0, nt=0, nis=0;
    for (int j=0; j<TAG; j++) {
      ni += nTotalInf[j];
      ns += nTotalSym[j];
      nt += nTotal[j];
      nis +=nTotalInfSample[j];
    }
    outfile << "Total infection attack rate: " << (ni/(double)nt) << endl;
   outfile << "Total infected sampled individuals: " << nis <<endl;  
    outfile << "Total infected individuals by age: ";
    for (int j=0; j<TAG; j++)
      outfile << nTotalInf[j] << ",";
    outfile << endl;
 outfile << "Total infected and sampled individuals by age: ";
    for (int j=0; j<TAG; j++)
      outfile << nTotalInfSample[j] << ",";
    outfile << endl;
...
void EpiModel::outputIndividuals(void) {
  if (bIndividualsFile) {
    ...
    out << "id,age,familyid,homecomm,homeneighborhood,daycomm,dayneighborhood,workplace,infectedtime,incubationdays,sourceid,sourcetype,vacstatus,sampled,knowPositive" << endl;
    for (vector< Person >::iterator it = pvec.begin();
       it != pvec.end();
       it++) {
      Person &p = *it;
     //if(isSampled(p)&&isKnownPositive(p)) knownisolated++;
	  ...
	  << p.nInfectedTime << "," << p.nIncubationDays << "," << p.sourceid << "," << (int) p.sourcetype << "," <<  (int) isVaccinated(p)<<","<< isSampled(p) << "," <<(int)p.knownPositive << endl; 
    }
...
void EpiModel::run(void) {
  prerun();
   ...
   for (vector< Community >::iterator it = commvec.begin();
          ...
          for (unsigned int pid=comm.nFirstPerson;pid<comm.nLastPerson;pid++) {
              Person &p = pvec[pid];
              if (p.nWorkplace >=1 && p.nWorkplace <=4) Z++; 
           }
   }      
   sprop =(double) sampledSize/(double)Z;
   cout<< "total kids" << Z <<" ratio "<<sprop<<endl;

  while(nTimer<nRunLength*2) {   
   //sample the population; if this person is infected, then mark it as isSampled, then this infected one will go through isolation method
     
      if(sampledDay > 0? (nTimer >= nTriggerTime-3 && (nTimer-nTriggerTime+3)%(sampledDay*2)==0):(nTimer == nTriggerTime-3)){
          //every 7(2*7) days to sampled,
          int nsampled = 0; 
          int nsampleandinfected= 0;
          nNumInfectedSampleddaily = 0;
          for (vector< Community >::iterator it = commvec.begin();
               it != commvec.end();
               it++) {
              Community &comm = *it;
              ...
              for (unsigned int pid=comm.nFirstPerson;pid<comm.nLastPerson;pid++) {
                  Person &p = pvec[pid];
                  if(p.nWorkplace ==1) school1++;
                  if(p.nWorkplace ==2) school2++;
                  if(p.nWorkplace ==3) school3++;
                  if(p.nWorkplace ==4) school4++;
              }
              int nsample1=0, nsample2=0,nsample3=1,nsample4 = 1;
              int nsampled1=0, nsampled2=0,nsampled3=0 ,nsampled4 = 0;
              double sampleprob1=0, sampleprob2=0, sampleprob3=0, sampleprob4 = 0;
              nsample1 = school1*sprop;
              if(nsample1 < 1) nsample1 = 1;

              nsample2 = school2*sprop;
              if(nsample2 < 1) nsample2 = 1;

              nsample3 = school3*sprop;
              if(nsample3 < 1) nsample3 = 1;

              nsample4 = school4*sprop;
              if(nsample4 < 1) nsample4 = 1;
              
               sampleprob1 =(double)nsample1/(double)school1;
               sampleprob2 =(double)nsample2/(double)school2;
               sampleprob3 =(double)nsample3/(double)school3;
               sampleprob4 =(double)nsample4/(double)school4;
              ...
              for (unsigned int pid=comm.nFirstPerson;pid<comm.nLastPerson;pid++) {
                  ...
                  cleanSampled(p);
                  double sampleprob= 0;
                  if(p.nWorkplace ==1) sampleprob=sampleprob1;
                  if(p.nWorkplace ==2) sampleprob=sampleprob2;
                  if(p.nWorkplace ==3) sampleprob=sampleprob3;
                  if(p.nWorkplace ==4) sampleprob=sampleprob4;

                  //cout<<sampleprob<<endl;
                  if(get_rand_double<=sampleprob){
                           if(p.nWorkplace ==1) nsampled1++;
                           if(p.nWorkplace ==2) nsampled2++;
                           if(p.nWorkplace ==3) nsampled3++;
                           if(p.nWorkplace ==4) nsampled4++;
                           nsampled++;
                          
                          if(isInfected(p)) {
                              setIsSampled(p);//which means sampled people is affected and will go to isolation method
                              nsampleandinfected++;
                              p.knownPositive = 1;
                              ++commvec[p.nHomeComm].ninfS[p.age];
                              setWillBeAscertained(p);
               ...
               for (unsigned int pid=comm.nFirstPerson;pid<comm.nLastPerson;pid++) {
                 ...
                 // cleanSampled(p);
                 onekid=0;
                  if(nsampled1==0&&p.nWorkplace==1) {nsampled1++;onekid=1;nsampled++;}
                  if(nsampled2==0&&p.nWorkplace==2) {nsampled2++;onekid=1;nsampled++;}                      
                  if(nsampled3==0&&p.nWorkplace==3) {nsampled3++;onekid=1;nsampled++;}
                  if(nsampled4==0&&p.nWorkplace==4) {nsampled4++;onekid=1;nsampled++;}
                   
                    if(onekid==1&&isInfected(p)) {
                              setIsSampled(p);//which means sampled people is affected and will go to isolation method
                              nsampleandinfected++;
                              p.knownPositive = 1;
                              ++commvec[p.nHomeComm].ninfS[p.age];
                ...
                //cout <<nTimer<<" commID "<<comm.id <<" Kids_pop "  << Z <<" SampledSize "<<sampledSize<<" ele "<<school1<<" ele "<<school2<<" middle "<<school3<<" high "<<school4<<" sampled "<< nsampled1<< " "<<nsampled2<<" " <<nsampled3<<" "<<nsampled4<<" infected "<<nsampleandinfected<<endl;

    
                //cout << "sampled pop: "<< Z <<" comm:" << comm.nNumResidents  <<" sampled:"<<nsampled<<" infected:"<<nsampleandinfected<<endl;
              }
      //cout << "sampled pop: "  <<" sampled:"<<nsampled1<< " "<<nsampled2<<" " <<nsampled3<<" "<<nsampled4<<" infected:"<<nsampleandinfected<<endl;
     
      nNumInfectedSampleddaily = nsampleandinfected;
      //nNumInfectedSampledDailyHistory.push_back( nsampleandinfected);
      //cout << "sampled pop: "<< sampledSize <<" comm;"<<Z  <<" infected:"<<nsampleandinfected<<endl;
    }
     
    //cout  << " infected&isolated: "<< nNumInfectedSampleddaily <<endl;
    nNumInfectedSampledDailyHistory.push_back(nNumInfectedSampleddaily);
  ...
  if (nLogFileInterval>0 && (int)(nTimer/2)%nLogFileInterval==0)
   ...
   unsigned int nNumInfectedSampled=0;    
   unsigned int nNumInfected=0;
   unsigned int nNumCumulativeInfected=0;
for (vector< Community >::iterator it = commvec.begin();
	 it != commvec.end();
	 it++) {
      for (int j=0; j<TAG; j++) {
	...
	nNumSymptomatic += (*it).nsym[j];
        nNumInfectedSampled += (*it).ninfS[j];
	nNumInfected += (*it).ninf[j];
        nNumCumulativeInfected +=(*it).nEverInfected[j];
      }
    }
 //   cout<<nTimer<<" day:"<<endl;
    nNumCumulativeSymptomaticHistory.push_back(nNumCumulativeSymptomatic);
    nNumSymptomaticHistory.push_back(nNumSymptomatic);
    nNumInfectedSampledHistory.push_back(nNumInfectedSampled);
    nNumInfectedHistory.push_back(nNumInfected);  
    nNumCumulativeInfectedHistory.push_back(nNumCumulativeInfected);
    //nNumInfectedSampledDailyHistory.push_back(nNumInfectedSampleddaily);
}
  if (logfile)
    (*logfile).close();
