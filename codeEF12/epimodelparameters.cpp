...
EpiModelParameters::EpiModelParameters(const char *configname) {
  ...
  nAVTotalLimit = nVaccineDailyLimit = nAVDailyLimit = UINT_MAX;
    ...
    fKnownPositiveFraction=0;
    fknownPositiveIsolation=0;
    sampledSize=0;
    sampledDay=0;
  ...
  fQuarantineComplianceSampled=0.0;
...
bool EpiModelParameters::readConfigFile(const char *configname) {
  ...
  if (!iss) {
    ...
    } else if (param.compare("antiviralpolicySampled")==0) {
          string s;
          if (iss>>s) {
            if (s.compare("none")==0)
              eAntiviralPolicySampled=NOAV;
            else if (s.compare("treatmentonly")==0)
              eAntiviralPolicySampled=TREATMENTONLY;
            else if (s.compare("HHTAP")==0)
              eAntiviralPolicySampled=HHTAP;
            else if (s.compare("HHTAP100")==0)
              eAntiviralPolicySampled=HHTAP100;
            else if (s.compare("FULLTAP")==0)
              eAntiviralPolicySampled=FULLTAP;
            else {
              cerr << "  " << s << " is not a valid antiviral strategy." << endl;
            }
          }
	...
	} else if(param.compare("knownPositiveFraction") == 0){
          read_config_double(fKnownPositiveFraction, iss, "Known Positive compliance probability", 0.0, 1.0);
        } else if(param.compare("knownPositiveIsolation") == 0){
	   read_config_double(fknownPositiveIsolation, iss, "Known Positive Isolation compliance probability", 0.0, 1.0);
        } else if(param.compare("sampledSize")==0){
          read_config_int(sampledSize, iss, "number of sampled people");
        } else if(param.compare("sampledDay")==0){
          read_config_int(sampledDay, iss, "every sample day");
        } else if(param.compare("poolSize")==0){
          read_config_int(poolSize, iss, "number of pool size");
        }

          else if (param.compare("AVEiSampled")==0) {
                    read_config_double(AVEiSampled, iss, "AVEiSampled and positive", 0.0, 1.0);
	...
	} else if (param.compare("quarantineSampled")==0) {
        read_config_double(fQuarantineComplianceSampled, iss, "quarantine compliance probability for sampled and positive", 0.0, 1.0);
